(() => {
  ...
  const w3Progress=name=>{
    ...
    const items=[
      ['Sandbag to shoulder','sb',false],
      ['Ground to air','gta',false],
      ['Armheving – beste runde','push',false],
      ['Total tid','total',true]
    ];
    return `<div class="hist"><div class="h">Resultat</div><div class="h">Første</div><div class="h">PB</div>${items.map(([label,key,low])=>{const v=firstAndBestW3(a,key,low);return row(label,v.first,v.best)}).join('')}</div>`;
  };
  ...
  const apply=name=>{
    ...
    if(p2){p2.innerHTML=`<h2>Sirkeløkt – progresjon</h2><div class="muted" style="margin-bottom:7px">Første registrerte resultat sammenlignes med din beste registrering.</div>${w3Progress(name)}`;}
  };
