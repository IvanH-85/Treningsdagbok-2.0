(() => {
  ...
  style.textContent=`
    .gtcal-card{overflow:hidden}.gtcal-head{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:10px}.gtcal-nav{display:flex;gap:6px;align-items:center}.gtcal-title{font-size:15px;font-weight:900;min-width:140px;text-align:center}
    .gtcal-grid{display:grid;grid-template-columns:34px repeat(7,minmax(0,1fr));gap:4px}.gtcal-dow{font-size:10px;font-weight:800;text-align:center;color:#6b7589;padding:3px 0}.gtcal-weekno{grid-column:1;grid-row:1/3;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:900;color:#6b7589;background:#f3f5f8;border-radius:7px}
    .gtcal-week{grid-column:1/-1;display:grid;grid-template-columns:34px repeat(7,minmax(0,1fr));grid-template-rows:auto auto;gap:4px;position:relative;margin-top:2px}.gtcal-rowdays{grid-column:2/-1;grid-row:2;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:4px}
    .gtcal-day{min-height:78px;border:1px solid var(--line);border-radius:9px;background:#fff;padding:5px;position:relative;overflow:hidden;cursor:pointer;transition:.12s}.gtcal-day:active{transform:scale(.98)}.gtcal-day.out{opacity:.35}.gtcal-day.today{outline:2px solid var(--b)}.gtcal-date{font-size:11px;font-weight:900;margin-bottom:4px;pointer-events:none}
  ...
  const allPlans=()=>{const out=[];['Ivan','Espen'].forEach(owner=>{const s=stateByName(owner)||{};(s.plans||[]).forEach((p,i)=>out.push({...p,_owner:owner,_index:i}))});return out};
  const workoutsForDate=date=>{const out=[];['Ivan','Espen'].forEach(name=>{const s=stateByName(name)||{};['w1','w2','w3','w4','w5'].forEach(k=>(s[k]||[]).forEach(x=>{if((x.date||'')===date)out.push({name,k,x})}))});return out};
  const workoutShort=k=>({w1:'Styrke 1',w2:'Kondisjon',w3:'Styrke 2',w4:'Kondisjon',w5:'Test'})[k]||'Økt';
  const workoutIcon=k=>(k==='w1'||k==='w3')?'💪🏼':(k==='w2'||k==='w4')?'🏃🏻‍♂️':k==='w5'?'🧪':'✅';
  const planIcon=t=>t==='Styrke 1'||t==='Styrke 2'?'💪🏼':t==='Kondisjon'?'🏃🏻‍♂️':t==='Test'?'🧪':'📅';

  const fetchChallenges=async()=>{
    if(typeof sb==='undefined')return;
    const r=await sb.from('coach_challenges').select('*').order('start_date',{ascending:true});
    if(r.error){console.error('Kunne ikke hente challenges til kalender',r.error);return}
    challengesCache=r.data||[];
  };

  const refreshChallengesAndCalendar=async()=>{await fetchChallenges();placeCalendar()};
  ...
  const challengeSegments=(weekStart,weekEnd)=>challengesCache.filter(ch=>ch.start_date<=ymd(weekEnd)&&ch.end_date>=ymd(weekStart)).map(ch=>{
    const s=fromYmd(ch.start_date),e=fromYmd(ch.end_date),start=Math.max(0,Math.round((new Date(Math.max(s,weekStart))-weekStart)/86400000)),end=Math.min(6,Math.round((new Date(Math.min(e,weekEnd))-weekStart)/86400000));
    return {ch,start,end};
  });
  ...
  const openModal=html=>{
    document.getElementById('gtcalModal')?.remove();
    const el=document.createElement('div');el.id='gtcalModal';el.className='gtcal-modal';el.onclick=e=>{if(e.target===el)el.remove()};
    el.innerHTML=`<div class="gtcal-modalbox"><button class="gtcal-close" onclick="document.getElementById('gtcalModal').remove()">×</button>${html}</div>`;
    document.body.appendChild(el);
  };

  window.openCalendarDay=date=>openModal(detailHtml(date));

  window.openCalendarChallenge=id=>{
    const ch=challengesCache.find(x=>Number(x.id)===Number(id));if(!ch)return;
    openModal(`<h2>🎯 ${esc(ch.title)}</h2><div class="muted">${esc(ch.start_date)} – ${esc(ch.end_date)}</div>${ch.message?`<p>${esc(ch.message)}</p>`:''}<div><b>Mål:</b> ${esc(ch.target)} ${esc(ch.unit)}</div><div><b>Til:</b> ${esc(ch.assignee)}</div>${ch.reward?`<div class="gtcal-chinfo">🎁 <b>Premie:</b> ${esc(ch.reward)}</div>`:''}`);
  };
  ...
  const calendarHtml=()=>{
    ...
    let html=`<div id="gtcalCard" class="card gtcal-card"><div class="gtcal-head"><h2 style="margin:0">Treningskalender</h2><div class="gtcal-nav"><button class="btn small secondary" onclick="calendarPrevMonth()">‹</button><div class="gtcal-title">${monthName(calCursor)}</div><button class="btn small secondary" onclick="calendarNextMonth()">›</button></div></div><div class="gtcal-grid"><div class="gtcal-dow">Uke</div>${['Man','Tir','Ons','Tor','Fre','Lør','Søn'].map(x=>`<div class="gtcal-dow">${x}</div>`).join('')}</div>`;
    weeks.forEach(weekStart=>{
      const weekEnd=addDays(weekStart,6),segments=challengeSegments(weekStart,weekEnd);
      html+=`<div class="gtcal-week"><div class="gtcal-weekno">${isoWeek(weekStart)}</div>${segments.map(({ch,start,end})=>`<div class="gtcal-challenge" style="grid-column:${start+2}/${end+3}" onclick="event.stopPropagation();openCalendarChallenge(${ch.id})">🎯 Challenge – ${esc(ch.title)}</div>`).join('')}<div class="gtcal-rowdays">`;
      for(let i=0;i<7;i++){
  ...
  const placeCalendar=()=>{
    ...
    const wrap=document.createElement('div');wrap.innerHTML=calendarHtml();const cal=wrap.firstElementChild,firstCard=home.querySelector(':scope > .card');
  ...
  window.savePlannedWorkout=async()=>{
    if(isCoach())return;
    const who=q('#gtPlanWho')?.value,date=q('#gtPlanDate')?.value,type=q('#gtPlanType')?.value,note=q('#gtPlanNote')?.value.trim();if(!date||!type)return;
    ...
    await save('Planlagt økt lagret');placeCalendar();openCalendarDay(date);
  };

  window.deletePlannedWorkout=async(owner,index)=>{
    if(isCoach()||owner!==currentProfile?.name)return;if(!Array.isArray(d.plans)||!d.plans[index])return;
    d.plans.splice(index,1);await save('Planlagt økt slettet');placeCalendar();
  };

  const baseRenderHome=renderHome;
  renderHome=function(...args){const out=baseRenderHome(...args);placeCalendar();setTimeout(()=>{if(currentProfile)refreshChallengesAndCalendar()},120);return out};
  const baseSave=save;
  save=async function(...args){const out=await baseSave(...args);placeCalendar();return out};
  const baseStartSession=startSession;
  startSession=async function(...args){const out=await baseStartSession(...args);await refreshChallengesAndCalendar();return out};

  if(typeof currentProfile!=='undefined'&&currentProfile){refreshChallengesAndCalendar()}
})();