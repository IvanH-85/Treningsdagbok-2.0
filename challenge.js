(() => {
  ...
  const loadChallenges=async()=>{
    ...
    if(!c.error) challenges=c.data||[];
    if(!p.error) challengeProgress=p.data||[];
  ...
  window.createChallenge=async()=>{
    if(!isCoach())return;
    const title=q('#chTitle')?.value.trim(),target=Number(q('#chTarget')?.value),assignee=q('#chAssignee')?.value,metric=q('#chMetric')?.value,unit=q('#chUnit')?.value.trim()||metricInfo[metric]?.unit||'stk',start=q('#chStart')?.value,end=q('#chEnd')?.value;
    if(!title||!target||!start||!end){alert('Fyll inn navn, mål og datoer.');return}
    await sb.from('coach_challenges').update({active:false}).eq('active',true);
    const r=await sb.from('coach_challenges').insert({title,metric,target,unit,assignee,team_mode:assignee==='Begge'?(q('#chTeamMode')?.value||'individual'):'individual',start_date:start,end_date:end,reward:q('#chReward')?.value.trim()||null,message:q('#chMessage')?.value.trim()||null,created_by:currentUser.id,active:true});
    if(r.error){alert('Kunne ikke opprette challenge: '+r.error.message);return}
    await loadChallenges();renderHome();showToast('Challenge publisert 💪');
  };
  ...
  if(typeof currentProfile!=='undefined'&&currentProfile){loadChallenges().then(()=>renderHome())}
})();
